...
namespace boson
{
    ...
    Router &Router::get(const std::string &path, const Middleware &middleware, const RouteHandler &handler)
    {
    ...
    Router &Router::get(const std::string &path, const std::vector<Middleware> &middlewares, const RouteHandler &handler)
    {
        return addRouteWithMiddleware("GET", path, handler, middlewares);
    }
    ...
    // Overload for middleware support
    Router &Router::post(const std::string &path, const Middleware &middleware, const RouteHandler &handler)
    {
        std::vector<Middleware> middlewares = {middleware};
    ...
    // Overload for multiple middleware
    Router &Router::post(const std::string &path, const std::vector<Middleware> &middlewares, const RouteHandler &handler)
    {
        return addRouteWithMiddleware("POST", path, handler, middlewares);
    }
    ...
    Router &Router::put(const std::string &path, const Middleware &middleware, const RouteHandler &handler)
    {
        std::vector<Middleware> middlewares = {middleware};
        return addRouteWithMiddleware("PUT", path, handler, middlewares);
    }

    Router &Router::put(const std::string &path, const std::vector<Middleware> &middlewares, const RouteHandler &handler)
    {
        return addRouteWithMiddleware("PUT", path, handler, middlewares);
    }
    ...
    Router &Router::del(const std::string &path, const Middleware &middleware, const RouteHandler &handler)
    {
        std::vector<Middleware> middlewares = {middleware};
        return addRouteWithMiddleware("DELETE", path, handler, middlewares);
    }

    Router &Router::del(const std::string &path, const std::vector<Middleware> &middlewares, const RouteHandler &handler)
    {
        return addRouteWithMiddleware("DELETE", path, handler, middlewares);
    }
    ...
    Router &Router::patch(const std::string &path, const Middleware &middleware, const RouteHandler &handler)
    {
        std::vector<Middleware> middlewares = {middleware};
        return addRouteWithMiddleware("PATCH", path, handler, middlewares);
    }

    Router &Router::patch(const std::string &path, const std::vector<Middleware> &middlewares, const RouteHandler &handler)
    {
        return addRouteWithMiddleware("PATCH", path, handler, middlewares);
    }
    ...
    Router &Router::addRouteWithMiddleware(const std::string &method, const std::string &path, const RouteHandler &handler, const std::vector<Middleware> &middleware)
    {
        Route route;
        route.method = method;
        route.path = path;
