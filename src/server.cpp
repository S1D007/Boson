...
struct Buffer {
    ...
    Buffer(size_t initialCapacity = 8192) 
...
public:
    Connection(socket_t socket, EventLoop* eventLoop, Router& router, 
        ...
        : socket_(socket), eventLoop_(eventLoop), router_(router),
          middleware_(middleware), errorHandler_(errorHandler),
          readBuffer_(16384), writeBuffer_(16384), keepAlive_(true),
...
public:
    ...
    bool addSocket(socket_t socket, bool isListening = false) {
        ...
        if (isListening) {
            ...
            auto connection = std::make_shared<Connection>(socket, this, router_, globalMiddleware_, errorHandler_);
    ...
    void updateEvents(socket_t socket, bool readable, bool writable) {
        ...
        if (readable) event.events |= EPOLLIN;
        if (writable) event.events |= EPOLLOUT;
...
private:
    void acceptNewConnections() {
        while (true) {
            ...
            socket_t clientSocket = accept(listenerSocket_, 
    ...
    void cleanupIdleConnections() {
        ...
        for (const auto& pair : connectionLastActive_) {
            if (now - pair.second > std::chrono::seconds(120)) { // 2 min timeout
                socketsToRemove.push_back(pair.first);
            }
        }
...
bool Connection::onReadable() {
    if (state_ != State::READ_REQUEST) return true;
    ...
    while (true) {
        ...
        if (bytesRead > 0) {
            ...
            if (readBuffer_.size > 1024 * 1024) { // 1MB limit
...
void Connection::processRequest() {
    if (state_ != State::PROCESS_REQUEST) return;
...
bool Connection::onWritable() {
    if (state_ != State::WRITE_RESPONSE || !isWriting_) return true;
    ...
    while (writeBuffer_.size > 0) {
        ...
        if (bytesSent > 0) {
            ...
            memmove(writeBuffer_.data, writeBuffer_.data + bytesSent, writeBuffer_.size - bytesSent);
