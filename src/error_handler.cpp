...
BadRequestError::BadRequestError(const std::string& message) : HttpError(message, 400) {}

UnauthorizedError::UnauthorizedError(const std::string& message) : HttpError(message, 401) {}

ForbiddenError::ForbiddenError(const std::string& message) : HttpError(message, 403) {}

NotFoundError::NotFoundError(const std::string& message) : HttpError(message, 404) {}
...
void defaultErrorHandler(const std::exception& err, const Request& req, Response& res)
{
    ...
    res.status(statusCode).header("Content-Type", "application/json").send(jsonResponse);
}
