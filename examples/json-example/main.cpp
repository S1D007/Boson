...
int main() {
    ...
    server.get("/api/user", [](const boson::Request& req, boson::Response& res) {
        res.jsonObject({
            {"id", 1},
            {"name", "John Doe"},
    ...
    server.get("/api/users", [](const boson::Request& req, boson::Response& res) {
        res.jsonObject({
            {"users", {
    ...
    server.get("/api/complex", [](const boson::Request& req, boson::Response& res) {
        res.jsonObject({
            {"user", {
                {"id", 1},
                {"name", "John Doe"},
