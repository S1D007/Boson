...
<body>
    ...
    <footer>
        <div class="container">
            <p>&copy; )" +
               (data.count("year") ? data.at("year") : "2025") + R"( )" +
               (data.count("project_name") ? data.at("project_name") : "{{.ProjectName}}") +
            ...
            for (const auto& user : users)
            {
                ...
                <div class="user-card" data-user-id=")" +
                    ...
                    <div class="user-actions">
                        <a href="/users/)" +
                           std::to_string(user.id) + R"(" class="btn btn-primary">View</a>
                        <a href="/users/)" +
                           std::to_string(user.id) + R"(/edit" class="btn btn-secondary">Edit</a>
                        <button class="btn btn-danger delete-user" data-user-id=")" +
                           std::to_string(user.id) + R"(">Delete</button>
                    </div>
                </div>)";
            }
        }
    ...
    static std::string renderUserForm(const User& user = {}, bool isEdit = false)
    {
        ...
        std::string content = R"(
            ...
            <form id="user-form" data-user-id=")" +
                              (isEdit ? std::to_string(user.id) : "") + R"(">
                <div class="form-group">
                    <label for="name">Name</label>
                    <input type="text" id="name" name="name" value=")" +
                              user.name + R"(" required>
                </div>
                
                <div class="form-group">
                    <label for="email">Email</label>
                    <input type="email" id="email" name="email" value=")" +
                              user.email + R"(" required>
                </div>
                
                <div class="form-group">
                    <label>
                        <input type="checkbox" id="active" name="active" )" +
                              (user.active ? "checked" : "") + R"(>
                        Active
                    </label>
                </div>
    ...
    static std::string renderUserDetail(const User& user)
    {
        std::string content = R"(
            <div class="user-detail">
                ...
                <div class="user-actions">
                    <a href="/users/)" +
                              std::to_string(user.id) + R"(/edit" class="btn btn-primary">Edit</a>
                    <button class="btn btn-danger delete-user" data-user-id=")" +
                              std::to_string(user.id) + R"(">Delete</button>
