...
class Controller
{
  ...
  protected:
     ...
     */
    void registerRoute(HttpMethod method, const std::string& path, const std::string& handlerName,
...
class ControllerFactory
{
  public:
     ...
     */
    template <typename T> static void registerController()
    {
     ...
     */
    template <typename T> static std::shared_ptr<T> createController()
    {
        static_assert(std::is_base_of<Controller, T>::value, "Type must inherit from Controller");
